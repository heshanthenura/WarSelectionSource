...
function startMapsListUpload()
  ...
  startUploading(mapsMyListUpload, "map editor maps list")
end
...
function startMapEditorFromCode()
  root.interface[parentInterface].f_sendEvent("mapEditorStartMap", toJson({
    code = mapCode
  }))
end

function updateMapEditorMapSize()
  local nodes = interface.nodes
  local v = nodes[38].widget_value
  if v == nodes[38].widget_lines_size - 1 then
    return
  end
  local mapSize = 135 + v * 70
  nodes[44].widget_text = mapSize
  nodes[46].widget_text = mapSize
end

updateMapEditorMapSize()
